...
function encodeDataInImage($data, $inputImagePath = null)
{
	if ($inputImagePath && file_exists($inputImagePath)) {
		...
		$imageFormat = strtolower(
			pathinfo($inputImagePath, PATHINFO_EXTENSION)
		);
	...
	for ($y = 0; $y < $height && $dataIndex < strlen($binaryData); $y++) {
		for ($x = 0; $x < $width && $dataIndex < strlen($binaryData); $x++) {
			...
			$colors["red"] =
				($colors["red"] & 0xfe) | (int) $binaryData[$dataIndex++];
			if ($dataIndex < strlen($binaryData)) {
				$colors["green"] =
					($colors["green"] & 0xfe) | (int) $binaryData[$dataIndex++];
			}
			if ($dataIndex < strlen($binaryData)) {
				$colors["blue"] =
					($colors["blue"] & 0xfe) | (int) $binaryData[$dataIndex++];
			}
